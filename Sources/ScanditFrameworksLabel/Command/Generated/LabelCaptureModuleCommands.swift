...
/// Finish callback for label capture did update session event
public class FinishValidationFlowResultUpdateEventCommand: LabelCaptureModuleCommand {
    private let module: LabelCaptureModule
    public init(module: LabelCaptureModule) {
        self.module = module
    }

    public func execute(result: FrameworksResult) {
        module.finishValidationFlowResultUpdateEvent(
            result: result
        )
    }
}
